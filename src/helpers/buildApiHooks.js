 ...
 * @param {Array} queryArray - The API actions.
 * @param {Function} baseQuery - The function to fetch the base query.
 * @param {Function} dispatchFn - The dispatch function from any context
 * @returns {{hookName:Function}} The API hooks.
 * @example
...
export const buildHooks = (queryArray, baseQuery, dispatchFn) => {
  ...
  queryArray.forEach((item) => {
    let hookName = `use${item.name[0].toUpperCase() + item.name.slice(1)}`;
